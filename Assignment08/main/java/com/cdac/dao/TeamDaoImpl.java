...
public class TeamDaoImpl implements TeamDao{
	...
	@Override
	public Team getTeamsAndPlayers(long teamId) {
		Session session=getSessionFactory().getCurrentSession();
		String query="select t from Team t join fetch t.players where t.id=:teamid";
		Team team=null;
		Transaction tx=session.beginTransaction();
		try {
			team=session.createQuery(query, Team.class).setParameter("teamid", teamId).getSingleResult();
			tx.commit();
		}
		catch(RuntimeException r){
			if(tx != null)
